...
On `git push`, the `verify` hook runs the project's verify command: `lintr::lint_package()`, then `spelling::spell_check_package()`, then `rcmdcheck::rcmdcheck(args = "--as-cran", error_on = "warning")`. Server-side branch protection was unavailable on the GitHub plan, which is why this local pre-push gate was built as the stand-in for it. It is no longer the only layer: since 2026-08-01 `main` is a protected branch on the GitLab remote — `allow_force_push=false`, push and merge restricted to Maintainers — so the guarantee the hook was approximating now also exists server-side, on the remote actually being pushed to. The two are not redundant. Protection rejects a bad *push*; it has no opinion about whether the tree passes `--as-cran`.

`.github/workflows/R-CMD-check.yaml` mirrors that chain across platforms. **It has never run.** It was written on 2026-07-31, when the repository had no reachable remote at all. A remote arrived on 2026-08-01 — a private GitLab project, now `origin` — and the claim survives it unchanged, which is worth stating plainly because the arrival of a working remote looks like the thing that would retire it. GitLab does not read `.github/workflows/`, and no `.gitlab-ci.yml` exists yet, so the workflow is now waiting on the GitHub account rather than on any push. Until one of those two things changes, the local hook is not a stand-in for CI — it is the only gate there is (`RADD-vvdpvysm`). This file previously described the hook's chain as "the same chain CI runs" while no `.github/` directory existed at all; the workflow now exists so that sentence becomes true on the first push rather than remaining a claim with nothing behind it. It has since been linted statically: actionlint 1.7.12 reports zero problems, and the three judgement calls actionlint cannot make — an `http-user-agent` key set on the r-devel matrix row only, `needs: lint` against a DESCRIPTION carrying no `Config/Needs` field, and `--no-manual` in CI but not in the hook — were each resolved against the `r-lib/actions` sources and found correct as written, with `docs/ci-workflow-lint.md` as the transcript (`RADD-ushiurrv`). That makes the file well-formed and its inputs understood; it does not make it exercised.
...
- **Three of the six CI rows are now measured locally, and Windows is the whole of what is left.** `data-raw/check-matrix.sh` runs macOS release on the host, Ubuntu release and Ubuntu r-devel in `rocker/r-ver` containers under amd64 emulation, and reports **Status: 1 NOTE on all three** — only the expected `--as-cran` incoming note. The transcript is `docs/check-matrix.md` (`RADD-kblrnovf`), and it accounts for every row: `oldrel-1` and `oldrel-2` are skipped deliberately because they probe toward a floor already measured, and `windows-latest` is **unreachable from this machine and checked by nothing**. Two cautions carry forward. An emulated container is close to a runner and not the same thing — that run had to install `libuv1-dev` for a source-built `fs`, and to copy the tree in with `cp` because GNU tar 1.35 cannot extract into a subdirectory under emulation — so a defect in either of those is a harness defect, not a package one. And neither container row pins a snapshot date, deliberately: pinning would move the dependency closure alongside the thing under test, which is the mistake recorded in the bullet above.
