...
#
# .github/workflows/R-CMD-check.yaml declares six matrix rows and a verify job,
# and has never executed once: the GitHub account that hosts the repository has
# been suspended since 2026-07-20, the appeal is awaiting manual review, and the
# wait is open-ended. So the six rows are a claim about coverage with nothing
# behind them, while AGENTS.md concedes that the local gate checks one platform
# and one R version -- macOS arm64 on R 4.6.0. This script is the stand-in
# adopted for the time being. It is NOT a replacement for CI, and it cannot
# become one: one of the six rows is unreachable from a Mac at all.
#
...
\`${HEAD_COMMIT}\` (\`${HEAD_DESCRIBE}\`). Three of the six rows in
